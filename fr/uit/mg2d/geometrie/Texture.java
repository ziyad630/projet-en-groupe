...
public class Texture extends Rectangle {
	...
	/**
	 * Constructeur DE DÉBOGAGE pour charger depuis un objet File.
	 */
	public Texture ( java.io.File imageFile, Point a ) {
		super ( a, 0, 0 , false); // Appel au constructeur de Rectangle
		if (imageFile == null || !imageFile.exists() || !imageFile.canRead()) {
			System.out.println("[!] Erreur DÉBOGAGE : Fichier image invalide ou non lisible : " + (imageFile != null ? imageFile.getAbsolutePath() : "null"));
			throw new java.lang.IllegalArgumentException("Fichier image invalide pour Texture (débogage)");
		}
		try {
			img = ImageIO.read ( imageFile );
			if (img == null) {
				System.out.println ("[!] Erreur DÉBOGAGE : ImageIO.read a retourné null pour " + imageFile.getAbsolutePath());
				throw new IOException("ImageIO.read a retourné null");
			}
		} catch ( IOException e ) {
			System.out.println ("[!] Erreur DÉBOGAGE : Impossible de lire l'image depuis " + imageFile.getAbsolutePath() + "\n" + e);
			throw new java.lang.RuntimeException("Erreur lecture image débogage", e);
		}
		...
		setLargeur(largeur);
		setHauteur(hauteur);
		hitbox = new ArrayList<Dessin>(); // Initialiser hitbox
		super.setB ( new Point ( a.getX() + largeur, a.getY() + hauteur ) );
	}
